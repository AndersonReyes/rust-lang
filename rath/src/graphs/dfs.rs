...
/// Get the path from source to destination node if it exist using Depth first search
pub fn path_to<T: Graph>(graph: &T, source: usize, destination: usize) -> Option<Vec<usize>> {
    let mut stack: BinaryHeap<usize> = BinaryHeap::new();
...
mod test {
    ...
    #[test]
    fn dfs_path_to_returns_none_for_node_with_no_paths() {
        let mut graph = UndirectedSparseGraph::new();
        graph.add_edge(0, 1);
        graph.add_edge(1, 2);
        graph.add_edge(5, 5);
